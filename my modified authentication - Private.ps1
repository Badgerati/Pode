    ...
    return {
        ...
        if ($null -eq $header) {
            return @{
                ...
                Code    = 401
                Headers = @{'WWW-Authenticate' = "Bearer realm=`"$($options.Realm)`"" }
            }
        }
    ...
    return {
        ...
        if ($null -eq $cert) {
            ...
            return @{
                Message = $message
                Code    = 400
                Headers = @{'WWW-Authenticate' = "Bearer realm=`"$($options.Realm)`", error=`"invalid_request`", error_description=`"$message`"" }
            }
        }

        # ensure the cert has a thumbprint
        if ([string]::IsNullOrWhiteSpace($cert.Thumbprint)) {
            $message = 'Invalid client certificate supplied: missing thumbprint'
            return @{
                Message = $message
                Code    = 400
                Headers = @{'WWW-Authenticate' = "TLS realm=`"$($options.Realm)`", error=`"invalid_token`", error_description=`"$message`"" }
            }
        }
        ...
        if (($cert.NotAfter -lt $now) -or ($cert.NotBefore -gt $now)) {
            $message = 'Client certificate supplied is expired or not valid yet'
            return @{
                Message = $message
                Code    = 403
                Headers = @{'WWW-Authenticate' = "TLS realm=`"$($options.Realm)`", error=`"invalid_token`", error_description=`"$message`"" }
            ...
            try {
                ...
                if ($_.Exception.Message -ilike '*jwt*') {
                    return @{
                        ...
                        Code    = 400
                        Headers = @{'WWW-Authenticate' = "Bearer realm=`"$($options.Realm)`", error=`"invalid_request`", error_description=`"$($_.Exception.Message)`"" }
                    }
                }
...
function Get-PodeAuthBearerType {
    return {
        ...
        param($options)
        write-podehost "I'm here"
        # Define common WWW-Authenticate header with placeholders
        $authHeaderBase = "Bearer realm=`"$($options.Realm)`", error=`"{0}`", error_description=`"{1}`""
        ...
        if ($null -eq $header) {
            ...
            return @{
                ...
                Header    = @{'WWW-Authenticate' = $authHeaderBase -f 'invalid_request', $message }
            }
        }
        ...
        if ([string]::IsNullOrWhiteSpace($token)) {
            ...
            return @{
                ...
                Headers = @{'WWW-Authenticate' = $authHeaderBase -f 'invalid_request', $message }
            }
        }
            ...
            try {
                ...
                if ($_.Exception.Message -ilike '*jwt*') {
                    return @{
                        ...
                        Headers = @{'WWW-Authenticate' = $authHeaderBase -f 'invalid_token', $_.Exception.Message }
                    }
                }
...
.PARAMETER $options
    ...
    - Algorithm: The hashing algorithm used (e.g., MD5, SHA-256).
...
function Get-PodeAuthDigestType {
    return {
        param($options)
        write-podehost "I'm here1"
        # Define common WWW-Authenticate header with placeholders
        $authHeaderBase = "Digest realm=`"$($options.Realm)`", nonce=`"$($options.Nonce)`", algorithm=`"$($options.Algorithm)`", error=`"{0}`", error_description=`"{1}`""

        # Get the Authorization header - send challenge if missing
        $header = (Get-PodeHeader -Name 'Authorization')
        if ($null -eq $header) {
            $message = 'No Authorization header found'
            ...
            return @{
                Message = $message
                Code    = 401  # RFC 7616: Missing credentials should return 401
                Headers = @{'WWW-Authenticate' = $authHeaderBase -f 'invalid_request', $message }
            }
        }
        ...
        if ($atoms[0] -ine $options.HeaderTag) {
            ...
            return @{
                Message = $message
                Code    = 401  # RFC 7616: Wrong authentication scheme should return 401
                Headers = @{'WWW-Authenticate' = $authHeaderBase -f 'invalid_request', $message }
            }
        }
...
function Test-PodeAuthValidation {
    ...
    try {
        ...
        if ($NoMiddlewareAuthentication -and (($result.Code -eq 400) -or ($result.Code -eq 401))) {
        ...
        if ($result -isnot [hashtable]) {
            ...
            #       Add-PodeHeader -Name 'WWW-Authenticate' -Value $result.Headers['WWW-Authenticate']
            #      }
        }
        write-podehost $result -Explode
        ...
        if (($null -eq $result) -or ($result.Count -eq 0) -or (Test-PodeIsEmpty $result.User)) {
            ...
            Write-podehost  "validCode =$validCode"
            if ($validCode) {
                if ($null -eq $result) {
                    $result = @{}
                }
                ...
                if (![string]::IsNullOrWhiteSpace($auth.Scheme.Name) -and !$result.Headers.ContainsKey('WWW-Authenticate')) {
                    write-podehost 'Get-PodeAuthWwwHeaderValue'
                    ...
                    $authHeader = Get-PodeAuthWwwHeaderValue -Name $auth.Scheme.Name -Realm $auth.Scheme.Realm -Challenge $result.Challenge
                    write-podehost  $authHeader
                    $result.Headers['WWW-Authenticate'] = $authHeader
                    write-podehost 'no Get-PodeAuthWwwHeaderValue'
                }
            }
