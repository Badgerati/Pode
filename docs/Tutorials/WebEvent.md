...
| AwaitingContent         | bool                 | If the request is chunked, this flags if Pode is still awaiting for the whole body to be sent                                                                                                                                        | -                             |
...
| ClientCertificateErrors | SslPolicyErrors      | Contains any errors that might have occurred while validating the client certificate. Pode ignores these by default, so they will need checking the [Client Certificate Authentication](../Authentication/Schemes/ClientCertificate) | -                             |
