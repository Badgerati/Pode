...
!!! important
    The `New-PodeAuthScheme` function is now deprecated, please use [`New-PodeAuthDigestScheme`](../../../../Functions/Authentication/New-PodeAuthDigestScheme) instead.

## Setup

To setup and start using Digest authentication in Pode you use the [`New-PodeAuthDigestScheme`](../../../../Functions/Authentication/New-PodeAuthDigestScheme) function, and then pipe this into the [`Add-PodeAuth`](../../../../Functions/Authentication/Add-PodeAuth) function. The parameters supplied to the [`Add-PodeAuth`](../../../../Functions/Authentication/Add-PodeAuth) function's ScriptBlock are the `$username`, and a HashTable containing the parameters from the Authorization header:

```powershell
Start-PodeServer {
    New-PodeAuthDigestScheme | Add-PodeAuth -Name 'Authenticate' -Sessionless -ScriptBlock {
...
By default, Pode will check if the Request's header contains an `Authorization` key, and whether the value of that key starts with `Digest` tag. The [`New-PodeAuthDigestScheme`](../../../../Functions/Authentication/New-PodeAuthDigestScheme) function can be supplied parameters to customise the tag using `-HeaderTag`. Pode will also gather the rest of the parameters in the header such as the Nonce, NonceCount, etc. An HTTP 401 challenge will be sent back if the Authorization header is invalid.
