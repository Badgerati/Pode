...
```powershell
 Set-PodeOARequest -RequestBody (New-PodeOARequestBody -ContentSchemas @{'application/octet-stream' = (New-PodeOAStringProperty -Format binary)})
```
```yaml
requestBody:
  content:
...
```powershell
 Set-PodeOARequest -RequestBody (New-PodeOARequestBody -ContentSchemas @{
...
```powershell
 Set-PodeOARequest -Properties -RequestBody (New-PodeOARequestBody -ContentSchemas @{
...
Examples:
```powershell
 Set-PodeOARequest -RequestBody  (New-PodeOARequestBody -ContentSchemas @{'multipart/form-data' =
...
```powershell
Add-PodeOAResponse -StatusCode 200 -Description 'A complex object array response' -ContentArray -Content (@{  'application/json' = 'VeryComplexType'  })
```
```json
{
  "200": {
...
```powershell
Add-PodeOAResponse -StatusCode 200 -Description 'A simple string response' -ContentArray -Content (@{  'text/plain' =  New-PodeOAStringProperty })
```
or
```powershell
Add-PodeOAResponse -StatusCode 200 -Description 'A simple string response' -ContentArray -Content (@{  'text/plain' =  'string' })
```
```json
{
