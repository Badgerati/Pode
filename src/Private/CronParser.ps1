...
function ConvertFrom-PodeCronExpression {
    ...
    if ($atoms.Length -ne 5) {
        throw ($PodeLocale.cronExpressionInvalidExceptionMessage -f $Expression) #"Cron expression should only consist of 5 parts: $($Expression)"
    }
    ...
    for ($i = 0; $i -lt $atoms.Length; $i++) {
        ...
        if (@('month', 'dayofweek') -icontains $_field) {
            while ($_atom -imatch $aliasRgx) {
                ...
                if ($null -eq $_alias) {
                    throw ($PodeLocale.invalidAliasFoundExceptionMessage -f $_field, $Matches['tag']) #"Invalid $($_field) alias found: $($Matches['tag'])"
                }
        ...
        if (!($_atom -imatch '^[\d|/|*|\-|,r]+$')) {
            throw ($PodeLocale.invalidAtomCharacterExceptionMessage -f $_atom)#"Invalid atom character: $($_atom)"
        }
        ...
        # error
            ...
            throw ($PodeLocale.invalidAtomCharacterExceptionMessage -f $_atom)#"Invalid cron atom format found: $($_atom)"
        }

        # ensure cron expression values are valid
        if ($null -ne $_cronExp.Range) {
            if ($_cronExp.Range.Min -gt $_cronExp.Range.Max) {
                throw ($PodeLocale.minValueGreaterThanMaxExceptionMessage -f $_field) #"Min value for $($_field) should not be greater than the max value"
            }

            if ($_cronExp.Range.Min -lt $_constraint[0]) {
                throw ($PodeLocale.minValueInvalidExceptionMessage -f $_cronExp.Range.Min, $_field, $_constraint[0]) #"Min value '$($_cronExp.Range.Min)' for $($_field) is invalid, should be greater than/equal to $($_constraint[0])"
            }

            if ($_cronExp.Range.Max -gt $_constraint[1]) {
                throw ($PodeLocale.maxValueInvalidExceptionMessage -f $_cronExp.Range.Max, $_field, $_constraint[1]) #"Max value '$($_cronExp.Range.Max)' for $($_field) is invalid, should be less than/equal to $($_constraint[1])"
            }
        }
        ...
        if ($null -ne $_cronExp.Values) {
            $_cronExp.Values | ForEach-Object {
                if ($_ -lt $_constraint[0] -or $_ -gt $_constraint[1]) {
                    throw ($PodeLocale.valueOutOfRangeExceptionMessage -f $value, $_field, $_constraint[0], $_constraint[1]) #"Value '$($_)' for $($_field) is invalid, should be between $($_constraint[0]) and $($_constraint[1])"
                }
            }
        }
        ...
        foreach ($mon in $cron['Month'].Values) {
            foreach ($day in $cron['DayOfMonth'].Values) {
                if ($day -gt $constraints.DaysInMonths[$mon - 1]) {
                    throw ($PodeLocale.daysInMonthExceededExceptionMessage -f $constraints.Months[$mon - 1], $constraints.DaysInMonths[$mon - 1], $day) #"$($constraints.Months[$mon - 1]) only has $($constraints.DaysInMonths[$mon - 1]) days, but $($day) was supplied"
                }
            }
        }
    }
