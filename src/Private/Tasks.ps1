...
function Start-PodeTaskHousekeeper {
    ...
    Add-PodeTimer -Name '__pode_task_housekeeper__' -Interval $PodeContext.Server.HouseKeeping.Tasks.TimerInterval -ScriptBlock {
        if ($PodeContext.Tasks.Results.Count -eq 0) {
            return
        }
        $RetentionMinutes= $PodeContext.Server.HouseKeeping.Tasks.RetentionMinutes
        $now = [datetime]::UtcNow
