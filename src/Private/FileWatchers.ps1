...
function New-PodeFileWatcher {
    ...
    $watcher.ErrorLoggingEnabled = (Test-PodeLoggerEnabled -Type Error)
    $watcher.ErrorLoggingLevels = If ( $listener.ErrorLoggingEnabled) { @(Get-PodeLoggerLevel -Type Error) } else { @() }
    return $watcher
}
