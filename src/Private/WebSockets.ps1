...
function New-PodeWebSocketReceiver {
    ...
    try {
        ...
        $receiver.ErrorLoggingEnabled = (Test-PodeLoggerEnabled -Type Error)
        $receiver.ErrorLoggingLevels = If ( $listener.ErrorLoggingEnabled) { @(Get-PodeLoggerLevel -Type Error) } else { @() }
