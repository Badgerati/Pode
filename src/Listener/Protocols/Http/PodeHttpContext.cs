...
namespace Pode.Protocols.Http
{
    ...
    public class PodeHttpContext : PodeContext, IDisposable
    {
        ...
        // Convenience property to get the listener as a PodeHttpListener
        private PodeHttpListener HttpListener => GetListener<PodeHttpListener>();
        ...
        public PodeHttpContext(Socket socket, PodeSocket podeSocket, IPodeListener listener)
        ...
        protected override async Task<bool> HandleRequestType()
        {
            ...
            {
                await UpgradeToWebSocket(PodeClientConnectionScope.Global, string.Empty, Request.GetStrategy<PodeHttpRequestStrategy>().Url.AbsolutePath, string.Empty, HttpListener.TrackClientConnectionEvents).ConfigureAwait(false);
                return false;
            }
        ...
        public async Task<PodeSignal> UpgradeToWebSocket(PodeClientConnectionScope scope, string clientId, string name, string group, bool trackEvents)
        {
            ...
            // add it to the listener
            HttpListener.AddSignalConnection(Signal);
