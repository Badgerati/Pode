...
namespace Pode.Protocols.Http.Client
{
    public class PodeClientConnection : IDisposable
    {
        ...
        public PodeHttpContext Context { get; private set; }
        public PodeHttpRequestStrategy Request => Context?.Request.GetStrategy<PodeHttpRequestStrategy>();
        ...
        protected PodeHttpListener HttpListener => Context?.GetListener<PodeHttpListener>();

        public PodeClientConnection(PodeClientConnectionType type, PodeHttpContext context, string name, string group, string clientId, PodeClientConnectionScope scope, bool trackEvents)
        {
            ConnectionType = type;
            Context = context;
            Name = name;
