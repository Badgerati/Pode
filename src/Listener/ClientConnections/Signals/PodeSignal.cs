...
namespace Pode.ClientConnections.Signals
{
    public class PodeSignal : PodeClientConnection
    {
        ...
        public PodeSignal(PodeContext context, string name, string group, string clientId, PodeClientConnectionScope scope, bool trackEvents)
        ...
        public override async Task Close()
        {
            ...
            await Send(new PodeSignalEnvelope(string.Empty, PodeWsOpCode.Close)).ConfigureAwait(false);
        ...
        {
            await Send(new PodeSignalEnvelope(string.Empty, PodeWsOpCode.Pong)).ConfigureAwait(false);
        }

        protected override async Task<bool> Ping()
        {
            return await Send(new PodeSignalEnvelope(string.Empty, PodeWsOpCode.Ping)).ConfigureAwait(false);
        }

        private async Task<bool> SendFrame(string message, PodeWsOpCode opCode = PodeWsOpCode.Text, bool flush = false)
        {
        ...
        public override void Dispose()
        {
            ...
            lock (Lockable)
            {
                ...
                Context.Listener.RemoveSignalConnection(this);
                base.Dispose();
