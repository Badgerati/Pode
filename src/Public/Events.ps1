...
function Register-PodeEvent {
    ...
    if (Test-PodeEvent -Type $Type -Name $Name) {
        throw ($msgTable.eventAlreadyRegisteredExceptionMessage -f $Type, $Name) # "$($Type) event already registered: $($Name)"
    }
...
function Unregister-PodeEvent {
    ...
    if (!(Test-PodeEvent -Type $Type -Name $Name)) {
        throw ($msgTable.noEventRegisteredExceptionMessage -f $Type, $Name) # "No $($Type) event registered: $($Name)"
    }
